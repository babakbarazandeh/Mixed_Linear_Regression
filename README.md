...
In this section, we evaluate the performance of the proposed method in estimating the regressor components in the MLR problem under different noise structures. In this experiment, we consider $K \in \{2,\cdots, 14\}$ components and $d \in \{1,\cdots, 5\}$  dimension for MLR. 
For each pair $(K,d)$, we first generate the $K$ regressors which result in $N = 20000$ samples, i.e.,  $\boldsymbol{\beta}_k^{*} \sim N(0, I_d)$ and $\textbf{x}_{i} \sim N(0, I_d),\;\forall i \in \{1,\cdots, N\},\;k \in \{1,\cdots,K\}$. 
